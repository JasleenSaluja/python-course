...
class Editor:
	def __init__(self, land_tiles, switch):
		...
		# clouds
		self.current_clouds = []
		self.cloud_surf = import_folder('super_mario/graphics/cloud')
		...
		# music
		self.editor_music = pygame.mixer.Sound('super_mario/audio/Explorer.ogg')
	...
	def imports(self):
		...
		self.sky_handle_surf = load('super_mario/graphics/cursor/handle.png').convert_alpha()
	...
	def save_grid_to_file(self,level_name="level_1"):  #new
		path=f'super_mario/levels/{level_name}.game'  #new
		with open(path,'wb') as f:                   #new
			self.canvas_data=pickle.dump(self.canvas_data, f)  #new
